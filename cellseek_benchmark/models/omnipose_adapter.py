...
def _import_omni_model_class():
    ...
    try:
        ...
        raise ImportError(
            "Omnipose failed to import. Use conda env `omnipose_env` (not `cellseek-benchmark`) "
            "with `pip install -e /path/to/omnipose`, or set PYTHONPATH to .../omnipose/src "
            "and install Omnipose dependencies (ocdkit, torch, etc.)."
        ) from e
