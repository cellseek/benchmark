...
def macro_metric_mean(rows: list[dict], keys: tuple[str, ...] = ("HOTA", "DetA", "AssA", "TA")) -> dict:
    out: dict = {}
    for k in keys:
...
def build_tracking_summary(
    ...
    *,
    ...
    valid_rows = [
        r
        for r in per_seq
        if "HOTA" in r and "DetA" in r and "AssA" in r and "TA" in r and r.get("HOTA") == r.get("HOTA")
    ]
